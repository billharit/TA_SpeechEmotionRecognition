...
def reverse_label_encoder(data):
    mapping = {'angry': 0, 'fear': 1, 'disgust': 2,
...
def mfcc_formula(audio):
    ...
    mfcc = np.mean(librosa.feature.mfcc(y=y, sr=sr, n_mfcc=13,
                   n_fft=n_fft, hop_length=hop_length).T, axis=0)
...
def LeNet_prediction(input):
    ...
    optimizer = torch.optim.SGD(
        model.parameters(), lr=0.01, weight_decay=0.001, momentum=0.8)
...
def get_features(waveforms, features, sample_rate):
    ...
    for waveform in waveforms:
        ...
        print(
            '\r'+f' Processed {file_count}/{len(waveforms)} waveforms', end='')
    return features
